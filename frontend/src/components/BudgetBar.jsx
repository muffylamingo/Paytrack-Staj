        ...
        <span className="flex items-center gap-2 font-medium text-bark-800">
          ...
          {budget.state !== 'İyi' && (
            <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${tone.chip}`}>
              {budget.state === 'Aşıldı' ? '⚠ Bütçe aşıldı' : '⚠ Bütçeye yaklaşıldı'}
            </span>
          )}
        </span>
        ...
        {showRemaining && (
          <span className={Number(budget.remaining) < 0 ? tone.text : ''}>
            {Number(budget.remaining) < 0
              ? `${formatCurrency(Math.abs(budget.remaining), budget.currency)} aşım`
              : `${formatCurrency(budget.remaining, budget.currency)} kaldı`}
          </span>
        )}
      </div>
    </div>
